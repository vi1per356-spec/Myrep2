...
# ---------- Адмін: прикріплення кнопки до повідомлення ----------
_admin_pending_msg = {}   # ADMIN_ID -> msg state dict
_custom_buttons   = []    # list of {'text': str, 'url': str}
CUSTOM_BTNS_FILE  = 'custom_buttons.json'

def _load_custom_btns():
...
@bot.message_handler(
    func=lambda msg: RUKLA1_OK
                     ...
                     and not (admin_input_pending and msg.from_user.id == ADMIN_ID)
                     and get_user_data(msg.chat.id).get('support_state') not in ('active', 'composing')
...
def support_menu(call):
    uid = call.message.chat.id
    data = get_user_data(uid)
    if data['support_state'] == 'active':
        bot.answer_callback_query(call.id, "Ви вже в діалозі. Використовуйте /close для завершення.")
        return
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(
        types.InlineKeyboardButton(T(uid,'support_btn_start'), callback_data="sup_start"),
        get_back_btn(uid),
    )
    bot.edit_message_text(T(uid,'support_intro'), uid, call.message.message_id,
                          reply_markup=markup)
...
def support_start(call):
    ...
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(get_back_btn(uid, 'm_main'))
    bot.edit_message_text(T(uid,'support_start_chat'), uid, call.message.message_id,
                          reply_markup=markup)

@bot.message_handler(func=lambda msg: get_user_data(msg.chat.id).get('support_state') == 'composing',
...
def support_ticket_compose(message):
    ...
    if admin_current_ticket is None:
        msg_text = T(ADMIN_ID,'support_ticket_new').format(
            text=ticket_text, name=user.first_name, uid=uid, uname=uname
        )
        markup = types.InlineKeyboardMarkup(row_width=1)
        markup.add(
            types.InlineKeyboardButton(T(ADMIN_ID,'support_btn_accept'), callback_data=f"sup_accept_{uid}"),
            types.InlineKeyboardButton(T(ADMIN_ID,'support_btn_reject'), callback_data=f"sup_reject_{uid}"),
        )
        if message.photo:
            msg = bot.send_photo(ADMIN_ID, message.photo[-1].file_id, caption=msg_text, reply_markup=markup)
        elif message.document:
            msg = bot.send_document(ADMIN_ID, message.document.file_id, caption=msg_text, reply_markup=markup)
        else:
            msg = bot.send_message(ADMIN_ID, msg_text, reply_markup=markup)
        data['support_admin_msg_id'] = msg.message_id
        data['support_state'] = 'waiting'
        bot.send_message(uid, T(uid, 'support_msg_sent'))
    else:
        pending_tickets.append(uid)
        data['support_state'] = 'waiting_in_queue'
        bot.send_message(uid, "Наразі оператор зайнятий, ваша заявка буде розглянута найближчим часом.")
...
def support_admin_accept(call):
    ...
    data = get_user_data(uid)
    if data['support_state'] not in ('waiting', 'waiting_in_queue'):
        bot.answer_callback_query(call.id, "Заявка вже не актуальна.")
        return
    if admin_current_ticket is not None:
        bot.answer_callback_query(call.id, "Ви вже в діалозі. Завершіть поточний /close.")
        return
    admin_current_ticket = uid
    data['support_state'] = 'active'
    data['support_chat_id'] = ADMIN_ID
    bot.send_message(uid, T(uid,'support_accepted'))
    bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
    bot.send_message(ADMIN_ID, f"Розпочато діалог з {uid}. Використовуйте /close для завершення.")

@bot.callback_query_handler(func=lambda call: call.data.startswith('sup_reject_'))
def support_admin_reject(call):
    global pending_tickets, admin_current_ticket
    if call.from_user.id != ADMIN_ID: return
    uid = int(call.data.split('_')[2])
    data = get_user_data(uid)
    data['support_state'] = None
    bot.send_message(uid, T(uid,'support_rejected'), reply_markup=types.InlineKeyboardMarkup().add(
        types.InlineKeyboardButton(T(uid,'support_btn_again'), callback_data='sup_start'),
        get_back_btn(uid)
    ))
    bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
...
def check_pending_tickets():
    ...
    if admin_current_ticket is None and pending_tickets:
        ...
        data = get_user_data(uid)
        if data['support_state'] == 'waiting_in_queue':
            ...
            if msg_type == 'photo' and file_id:
                msg = bot.send_photo(ADMIN_ID, file_id, caption=msg_text, reply_markup=markup)
            elif msg_type == 'document' and file_id:
                msg = bot.send_document(ADMIN_ID, file_id, caption=msg_text, reply_markup=markup)
            else:
                msg = bot.send_message(ADMIN_ID, msg_text, reply_markup=markup)
            data['support_admin_msg_id'] = msg.message_id
            data['support_state'] = 'waiting'
...
# ---------- Діалог підтримки ----------
@bot.message_handler(func=lambda msg: get_user_data(msg.chat.id).get('support_state') == 'active' and msg.text != '/close',
                     content_types=['text', 'photo', 'document'])
def support_chat_user(msg):
    uid = msg.chat.id
    if msg.photo:
        bot.send_photo(ADMIN_ID, msg.photo[-1].file_id, caption=f"Від користувача {uid}")
    elif msg.document:
        bot.send_document(ADMIN_ID, msg.document.file_id, caption=f"Від користувача {uid}")
    else:
        bot.send_message(ADMIN_ID, f"Користувач {uid}:\n{msg.text}")
...
try:
    ...
    try:
        ...
        TRANSLATOR_OK = False
        print("⚠️ Бібліотеки перекладу не знайдено.")
        def translate_text(text, target_lang):
            return text
...
def close_ticket(msg):
    global admin_current_ticket
    uid = msg.chat.id
    # Delegate to rukla7 if user is in an order contact relay
    try:
    ...
    if uid == ADMIN_ID:
        if admin_current_ticket is not None:
            ...
            data['support_state'] = None
            reset_rukla1_state(user_id)
            bot.send_message(user_id, T(user_id, 'support_closed'),
                             reply_markup=get_main_menu(user_id))
            bot.send_message(ADMIN_ID, "Діалог завершено.",
                             reply_markup=get_main_menu(ADMIN_ID))
            admin_current_ticket = None
            check_pending_tickets()
        else:
            bot.send_message(ADMIN_ID, "Немає активного діалогу.")
    else:
        data = get_user_data(uid)
        if data['support_state'] == 'active':
            data['support_state'] = None
            reset_rukla1_state(uid)
            bot.send_message(ADMIN_ID, f"Користувач {uid} завершив діалог.")
            bot.send_message(uid, T(uid, 'support_closed'),
                             reply_markup=get_main_menu(uid))
...
def show_balance(call):
    ...
    text = T(uid,'balance').format(balance=balance)
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(
        types.InlineKeyboardButton(T(uid,'buy_url_m'),    callback_data="buy_url_m_99"),
        types.InlineKeyboardButton(T(uid,'buy_grp_m'),    callback_data="buy_grp_m_99"),
        types.InlineKeyboardButton(T(uid,'buy_url_y'),    callback_data="buy_url_y_1099"),
        types.InlineKeyboardButton(T(uid,'buy_grp_y'),    callback_data="buy_grp_y_1099"),
        types.InlineKeyboardButton(T(uid,'buy_mix_m'),    callback_data="buy_mix_m_149"),
        types.InlineKeyboardButton(T(uid,'buy_mix_y'),    callback_data="buy_mix_y_1399"),
        types.InlineKeyboardButton(T(uid,'buy_tiktok_m'), callback_data="buy_tiktok_m_79"),
        types.InlineKeyboardButton(T(uid,'buy_tiktok_y'), callback_data="buy_tiktok_y_869"),
        types.InlineKeyboardButton(T(uid,'buy_all'),      callback_data="buy_all_m_1999"),
        types.InlineKeyboardButton(T(uid,'buy_custom'),   callback_data="buy_custom"),
        get_back_btn(uid),
    )
    bot.edit_message_text(text, uid, call.message.message_id, reply_markup=markup)
...
def buy_with_balance(call):
    ...
    # parts: ['balbuy', type, period, days, price]
    ptype = parts[1]
    days = int(parts[3])
    price = int(parts[4])
    balance = data['balance']
    if balance < price:
        bot.answer_callback_query(call.id,
            f"❌ Недостатньо коштів. Баланс: {balance}$, потрібно: {price}$.", show_alert=True)
        return
    secs = days * 86400
    now = time.time()
    if ptype in ('url', 'mix'):
    ...
    data['balance'] -= price
    save_db()
    bot.answer_callback_query(call.id, f"✅ Підписку придбано за {price}$ з балансу!", show_alert=True)
    # Refresh balance page
    call.data = 'm_balance'
    show_balance(call)

def product_info(cb_data):
...
@bot.callback_query_handler(func=lambda call: call.data.startswith('buy_') and call.data != 'buy_custom')
def buy_subscription(call):
    uid = call.message.chat.id
    ptype, secs, amount = product_info(call.data)
    data = get_user_data(uid)
    ...
    text = T(uid,'payment_prompt').format(amount=amount)
...
def custom_amount_start(call):
    uid = call.message.chat.id
    msg = bot.send_message(uid, T(uid, 'custom_amount_prompt'))
    bot.register_next_step_handler(msg, custom_amount_process)

def custom_amount_process(message):
    uid = message.chat.id
    try:
        amount = int(message.text.replace('$','').strip())
        if amount < 10:
            bot.send_message(uid, T(uid, 'min_amount_error'))
            return
        data = get_user_data(uid)
        data['pending_payment'] = {
            'type': 'balance',
        ...
        bot.send_message(uid, T(uid,'payment_prompt').format(amount=amount),
                         reply_markup=_payment_markup(uid, amount))
    except ValueError:
        bot.send_message(uid, T(uid, 'invalid_amount_error'))
...
def _payment_markup(uid, amount):
    ...
    markup = types.InlineKeyboardMarkup(row_width=1)
    if not is_topup:
        markup.add(types.InlineKeyboardButton(T(uid, 'pay_from_balance'), callback_data="sub_pay_bal"))
    markup.add(
        types.InlineKeyboardButton("Monobank - переказ на картку", callback_data=f"pay_mono_{amount}"),
        types.InlineKeyboardButton("USDT (TRON - TRC-20)", callback_data=f"pay_trc20_{amount}"),
        types.InlineKeyboardButton("USDT (TON)", callback_data=f"pay_ton_{amount}"),
        types.InlineKeyboardButton("USDT (Ethereum - ERC-20)", callback_data=f"pay_erc20_{amount}"),
        types.InlineKeyboardButton("Bitcoin (BTC)", callback_data=f"pay_btc_{amount}"),
        types.InlineKeyboardButton("Ethereum", callback_data=f"pay_eth_{amount}"),
        get_back_btn(uid, "m_balance"),
    )
    return markup
...
def pay_balance_sub(call):
    ...
    if balance < amount:
        bot.answer_callback_query(
            call.id,
            T(uid, 'not_enough_balance').format(balance=balance, price=amount),
            show_alert=True,
        )
        return
    now = time.time()
    ...
    save_db()
    bot.answer_callback_query(call.id, T(uid, 'bought_with_balance').format(price=amount), show_alert=True)
    try:
        call.data = 'm_balance'
        show_balance(call)
    except Exception:
...
def payment_address(call):
    ...
    text = T(uid,'wallet_info').format(amount=amount, name=name, address=address)
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(
        types.InlineKeyboardButton(T(uid,'i_paid'), callback_data=f"paid_{method}_{amount}"),
        get_back_btn(uid, "m_balance"),
    )
...
def ask_screenshot(call):
    uid = call.message.chat.id
    msg = bot.send_message(uid, T(uid, 'send_screenshot'))
    bot.register_next_step_handler(msg, process_screenshot, call.data)

def process_screenshot(message, payment_info):
    uid = message.chat.id
    if not message.photo and not message.document:
        bot.send_message(uid, T(uid, 'not_screenshot'))
        return
    ...
    if pp.get('is_balance_topup'):
        ...
        ptype = pp.get('type')
        if ptype == 'url':
            desc = "купівлі місячної/річної URL реклами"
        elif ptype == 'grp':
            desc = "купівлі місячної/річної реклами по групах"
        elif ptype == 'tiktok':
            desc = "купівлі підписки на рекламу у TikTok"
        elif ptype == 'all':
            desc = "купівлі підписки на всі сервіси"
        else:
            desc = "купівлі URL+групи"
    caption = (
    ...
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(
        types.InlineKeyboardButton("✅ Підтвердити платіж", callback_data=f"adm_confirm_pay_{uid}_{amount}"),
        types.InlineKeyboardButton("❌ Відхилити платіж", callback_data=f"adm_reject_pay_{uid}"),
    )
    if message.photo:
        bot.send_photo(ADMIN_ID, message.photo[-1].file_id, caption=caption, reply_markup=markup)
    else:
        bot.send_document(ADMIN_ID, message.document.file_id, caption=caption, reply_markup=markup)
    bot.send_message(uid, T(uid, 'payment_wait'))

# ---------- Адмін підтвердження платежу ----------
...
def admin_ask_confirm(call):
    ...
    uid = int(parts[3])
    amount = parts[4]
    msg = bot.send_message(ADMIN_ID, T(ADMIN_ID, 'adm_payment_confirm'))
    bot.register_next_step_handler(msg, admin_final_confirm, uid, amount, call.message)

def admin_final_confirm(message, uid, amount, original_msg):
    if message.text.strip().lower() == 'так':
        ...
        if pp.get('is_balance_topup'):
            ...
            bot.send_message(uid, T(uid, 'payment_approved'))
            save_db()
        data.pop('pending_payment', None)
        bot.send_message(ADMIN_ID, "✅ Операцію підтверджено.")
        try:
            bot.edit_message_reply_markup(original_msg.chat.id, original_msg.message_id, reply_markup=None)
        except:
            pass
    else:
        bot.send_message(ADMIN_ID, "❌ Підтвердження скасовано.")
...
def admin_reject_pay(call):
    ...
    data = get_user_data(uid)
    data.pop('pending_payment', None)
    bot.send_message(uid, T(uid, 'payment_rejected'), reply_markup=types.InlineKeyboardMarkup().add(
        types.InlineKeyboardButton(T(uid,'b_support'), callback_data='m_support'),
        get_back_btn(uid)
    ))
    try:
        bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
...
def admin_info(call):
    ...
    for uid, d in users_db.items():
        if d['balance'] > 0 or d['sub_url_until'] != 0 or d['sub_groups_until'] != 0 or d.get('sub_tiktok_until', 0) != 0:
            try:
                user = bot.get_chat(uid)
            ...
            lines.append(
                f"{name} ({uid}) {uname}\n"
                f"┏ Баланс: {d['balance']}$\n"
                f"┣ URL до: {format_sub_time(uid, d['sub_url_until'])}\n"
                f"┣ Групи до: {format_sub_time(uid, d['sub_groups_until'])}\n"
                f"┗ TikTok до: {format_sub_time(uid, d.get('sub_tiktok_until', 0))}"
            )
    if not lines:
        lines = ["Немає активних користувачів."]
    markup = types.InlineKeyboardMarkup()
    markup.add(get_back_btn(ADMIN_ID))
    bot.edit_message_text("\n\n".join(lines)[:4000], ADMIN_ID, call.message.message_id, reply_markup=markup)
...
def admin_add_bal(call):
    ...
    msg = bot.send_message(ADMIN_ID, "Введіть айді користувача якому потрібно поповнити баланс:")
...
def _adm_add_bal_uid(message):
    ...
    try:
        ...
        admin_input_pending = False
        bot.send_message(ADMIN_ID, "❌ Невірний ID. Введіть число.")
        return
    get_user_data(ADMIN_ID)['admin_temp_bal_uid'] = u_id
    msg = bot.send_message(ADMIN_ID, f"Введіть суму для поповнення балансу користувача {u_id}:")
...
def _adm_add_bal_fin(message):
    ...
    try:
        ...
        u_id = get_user_data(ADMIN_ID).get('admin_temp_bal_uid')
        if not u_id:
            bot.send_message(ADMIN_ID, "❌ Помилка: не знайдено ID користувача.")
            return
        get_user_data(u_id)['balance'] += amount
        save_db()
        bot.send_message(ADMIN_ID, T(ADMIN_ID, 'adm_bal_added').format(uid=u_id, amount=amount))
        try:
            bot.send_message(u_id, T(u_id, 'adm_user_bal_added').format(amount=amount))
        except Exception:
            pass
    except Exception as e:
        bot.send_message(ADMIN_ID, f"❌ Помилка введення: {e}")
...
def admin_rem_bal(call):
    ...
    if call.from_user.id != ADMIN_ID: return
    admin_input_pending = True
    msg = bot.send_message(ADMIN_ID, "Введіть айді користувача у якого потрібно зняти баланс:")
...
def _adm_rem_bal_uid(message):
    ...
    try:
        ...
        bot.send_message(ADMIN_ID, "❌ Невірний ID. Введіть число.")
        return
    get_user_data(ADMIN_ID)['admin_temp_rem_uid'] = u_id
    msg = bot.send_message(ADMIN_ID, f"Введіть суму для зняття з балансу користувача {u_id}:")
...
def _adm_rem_bal_fin(message):
    ...
    try:
        amount = int(message.text.strip())
        u_id = get_user_data(ADMIN_ID).get('admin_temp_rem_uid')
        if not u_id:
            bot.send_message(ADMIN_ID, "❌ Помилка: не знайдено ID користувача.")
            return
        get_user_data(u_id)['balance'] -= amount
        save_db()
        bot.send_message(ADMIN_ID, T(ADMIN_ID, 'adm_bal_removed').format(uid=u_id, amount=amount))
    except Exception as e:
        bot.send_message(ADMIN_ID, f"❌ Помилка введення: {e}")
...
def admin_give_sub(call):
    ...
    if call.from_user.id != ADMIN_ID: return
    admin_input_pending = True
    msg = bot.send_message(ADMIN_ID, T(ADMIN_ID, 'adm_enter_id_sub'))
...
def _adm_give_sub_type(message):
    ...
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(
        types.InlineKeyboardButton(T(ADMIN_ID,'adm_sub_url'), callback_data="admsub_url"),
        types.InlineKeyboardButton(T(ADMIN_ID,'adm_sub_grp'), callback_data="admsub_grp"),
        types.InlineKeyboardButton(T(ADMIN_ID,'adm_sub_tiktok'), callback_data="admsub_tiktok"),
        types.InlineKeyboardButton(T(ADMIN_ID,'adm_sub_mix'), callback_data="admsub_mix"),
        types.InlineKeyboardButton(T(ADMIN_ID,'adm_sub_all'), callback_data="admsub_all"),
    )
    bot.send_message(ADMIN_ID, T(ADMIN_ID, 'adm_sub_type'), reply_markup=markup)
...
def admin_give_sub_time(call):
    ...
    admin_input_pending = True
    msg = bot.send_message(ADMIN_ID, T(ADMIN_ID, 'adm_enter_term'))
...
def _adm_give_sub_fin(message):
    ...
    if not uid or not sub_type: return
    end = get_subscription_end_text(sub_type, term)
    if end is None:
        bot.send_message(ADMIN_ID, "Невірний формат терміну.")
        return
    data = get_user_data(uid)
    ...
    save_db()
    bot.send_message(ADMIN_ID, T(ADMIN_ID, 'adm_sub_given').format(uid=uid))
    try:
        bot.send_message(uid, T(uid, 'adm_sub_notify'))
    except Exception:
        pass
...
def admin_del_sub(call):
    ...
    if call.from_user.id != ADMIN_ID: return
    admin_input_pending = True
    msg = bot.send_message(ADMIN_ID, T(ADMIN_ID, 'adm_del_sub_confirm'))
...
def _adm_del_sub_fin(message):
    ...
    try:
        ...
        save_db()
        bot.send_message(ADMIN_ID, T(ADMIN_ID, 'adm_del_sub_done').format(uid=u_id))
    except:
        bot.send_message(ADMIN_ID, "❌ Помилка введення.")
...
def admin_manage_users(call):
    ...
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(
        types.InlineKeyboardButton(T(uid, 'adm_give_sub_btn'), callback_data="adm_give_sub"),
        types.InlineKeyboardButton(T(uid, 'adm_del_sub_btn'),  callback_data="adm_del_sub"),
        types.InlineKeyboardButton(T(uid, 'adm_add_bal_btn'),  callback_data="adm_add_bal"),
        types.InlineKeyboardButton(T(uid, 'adm_rem_bal_btn'),  callback_data="adm_rem_bal"),
        types.InlineKeyboardButton(T(uid, 'b_back_prev'),      callback_data="m_main"),
    )
    try:
        bot.edit_message_text(T(uid, 'b_manage_users'), uid, call.message.message_id,
                              reply_markup=markup)
    except Exception:
        bot.send_message(uid, T(uid, 'b_manage_users'), reply_markup=markup)
...
def admin_send_msg(call):
    if call.from_user.id != ADMIN_ID:
        ...
        return
    bot.answer_callback_query(call.id)
    msg = bot.send_message(ADMIN_ID, T(ADMIN_ID, 'send_msg_ask_uid'))
...
def _adm_send_msg_uid(message):
    ...
    try:
        ...
        bot.send_message(ADMIN_ID, T(ADMIN_ID, 'send_msg_user_not_found'))
        return
    if target_uid not in users_db:
        bot.send_message(ADMIN_ID, T(ADMIN_ID, 'send_msg_user_not_found'))
        return
    msg = bot.send_message(ADMIN_ID, T(ADMIN_ID, 'send_msg_ask_text'))
...
def _adm_send_msg_text(message, target_uid):
    ...
    _ask_admin_button(ADMIN_ID)


@bot.callback_query_handler(func=lambda call: call.data == 'adm_broadcast')
def admin_broadcast(call):
    if call.from_user.id != ADMIN_ID: return
    msg = bot.send_message(ADMIN_ID, T(ADMIN_ID, 'adm_broadcast_prompt'))
...
# ---------- Хелпери прикріплення кнопки ----------
def _ask_admin_button(uid: int):
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(
        types.InlineKeyboardButton(T(uid, 'btn_attach_create_new'),    callback_data='adm_btn_create'),
        types.InlineKeyboardButton(T(uid, 'btn_attach_pick_existing'), callback_data='adm_btn_pick'),
        types.InlineKeyboardButton(T(uid, 'btn_attach_none'),          callback_data='adm_btn_none'),
    )
    bot.send_message(uid, T(uid, 'btn_attach_ask'), reply_markup=markup)


def _do_send_admin_msg(markup):
    """Actually sends the pending admin message (to user or broadcast) with the given reply_markup."""
    state = _admin_pending_msg.pop(ADMIN_ID, None)
    if not state:
        return
    ...
    if state['mode'] == 'send_to_user':
        ...
        if _send_one(target):
            bot.send_message(ADMIN_ID, T(ADMIN_ID, 'send_msg_sent').format(uid=target))
        else:
            bot.send_message(ADMIN_ID, T(ADMIN_ID, 'send_msg_failed').format(err='send failed'))
    else:  # broadcast
        sent = errors = 0
        for uid in list(users_db.keys()):
            if _send_one(uid):
                ...
                errors += 1
        bot.send_message(ADMIN_ID, T(ADMIN_ID, 'adm_broadcast_result').format(sent=sent, errors=errors))
...
def admin_btn_create(call):
    ...
    global admin_input_pending
    admin_input_pending = True
    msg = bot.send_message(ADMIN_ID, T(ADMIN_ID, 'btn_create_ask_text'))
...
def _admin_btn_got_text(message):
    ...
    if message.from_user.id != ADMIN_ID:
        admin_input_pending = False
        return
    state = _admin_pending_msg.get(ADMIN_ID)
    if not state:
        admin_input_pending = False
        return
    state['_btn_text'] = (message.text or '').strip()
    msg = bot.send_message(ADMIN_ID, T(ADMIN_ID, 'btn_create_ask_url'))
...
def _admin_btn_got_url(message):
    ...
    # Save the button for future reuse
    _load_custom_btns()
...
def admin_btn_pick(call):
    ...
    markup.add(types.InlineKeyboardButton(T(ADMIN_ID, 'btn_attach_none'), callback_data='adm_btn_none'))
    bot.send_message(ADMIN_ID, "Виберіть кнопку:", reply_markup=markup)
